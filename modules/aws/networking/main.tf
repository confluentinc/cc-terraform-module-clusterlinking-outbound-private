...
resource "aws_vpc_endpoint_service" "brokers" {
  ...
  provisioner "local-exec" {
    when    = destroy
    command = <<EOT
    VPC_ENDPOINTS=$(aws ec2 describe-vpc-endpoint-connections --filters "Name=service-id,Values=${self.id}" --query "VpcEndpointConnections[].VpcEndpointId" --output text | tr '\t' ',')
    aws ec2 reject-vpc-endpoint-connections --service-id ${self.id} --vpc-endpoint-ids $VPC_ENDPOINTS
    EOT
  }
}
