# AWS — Cluster Linking outbound Private Link

Configures Confluent Cloud outbound Private Link networking so Cluster Linking can reach a self-managed Kafka cluster (Apache Kafka or MSK) running in an AWS VPC.

> **This is the repository _root_ module.** Consume it with `source = "git::https://github.com/confluentinc/cc-terraform-module-clusterlinking-outbound-private.git"` (no `//` subpath) and `use_aws = true`. This `modules/aws/` directory holds the AWS `networking` submodule the root module calls, plus this documentation. (Azure is a separate entry module — see [`../azure/README.md`](../azure/README.md).)
...
| Variable | Default | Type | Implied dependency | Description |
|---|---|---|---|---|
...
| `use_aws` | `false` | `bool` |  | Set module to create AWS specific resources |
